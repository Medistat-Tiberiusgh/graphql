...
export class PrescriptionsService {
  ...
  private validateNumericFields(
    ...
    if (numberOfPrescriptions < 0)
      errors.push('numberOfPrescriptions must be >= 0');
    if (numberOfPatients < 0) errors.push('numberOfPatients must be >= 0');
    if (per1000 < 0) errors.push('per1000 must be >= 0');
    if (numberOfPrescriptions < numberOfPatients)
      errors.push('numberOfPrescriptions must be >= numberOfPatients');
    return errors;
  }
  ...
  async create(
    ...
    if (year < 2006 || year > currentYear)
      errors.push(`year must be between 2006 and ${currentYear}`);
    errors.push(
    ...
    if (regionRows.length === 0)
      errors.push(`region with id ${region} does not exist`);
    if (drugRows.length === 0)
      errors.push(`drug with atcCode ${atcCode} does not exist`);
  ...
  async update(
    ...
    const errors = this.validateNumericFields(
      numberOfPrescriptions,
      numberOfPatients,
      per1000,
    );
